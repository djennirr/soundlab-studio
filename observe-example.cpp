...
//крутые структуры нам такие же нужны мб не все
enum class PinType
{
    Flow,
    Bool,
...
//разбивает на блоки в гуи эдиторе но точно хз как работает
    ...
    public Application
{
    ...
    //короче тут менюшка которая слева была она нам не нужна дай бог
    ...
    //для билда всех нод сразу которые есть
    // void BuildNodes()
    // {
    //     for (auto& node : m_Nodes)
    //         BuildNode(&node);
    ...
    //стартовое состояние нашей гуи инитим че надо и спавним че надо 
    //так сказать инициализируем рабочую область нашу 
    //по ощущениям тут есть лишнего много
    ...
    void OnStop() override
    {
        ...
        //отпустили текстуры из заточения
        //а оно нам надо вообще? эти ваши текстуры
    ...
    //это для цвета пинов или текстур хз
    ...
    //это иконки пинов разные нам пока не надо это
    ...
    // это тот самый стайл эдитор который открыть можно и в нем потыкать стили поменять цвета и тд нам оно не надо
...
//     }
    ...
    # endif
        ...
        ed::Begin("Node editor");
        {
            ...
            //проходимся по нодам которые есть
            for (auto& node : m_Nodes)
            {
                //не пон почему и блюпринт и симпл ноды????
            ...
            // здесь прошлись по нодам и отрисовали? те которые дерево
            for (auto& node : m_Nodes)
            {
            ...
            //здесь тоже отрисовали? те которые гудини
            for (auto& node : m_Nodes)
            {
    ...
    //контекстные меню разные которые открываются в зависимости от того куда мы жмали
    # if 1
        auto openPopupPosition = ImGui::GetMousePos();
        ed::Suspend();
        //когда на ноду нажали пкм
        if (ed::ShowNodeContextMenu(&contextNodeId))
            ImGui::OpenPopup("Node Context Menu");
        //когда на пин нажали пкм
        else if (ed::ShowPinContextMenu(&contextPinId))
            ImGui::OpenPopup("Pin Context Menu");
        //когда на проводок нажали
        else if (ed::ShowLinkContextMenu(&contextLinkId))
            ImGui::OpenPopup("Link Context Menu");
        //когда на поле нажали пкм
        ...
        //менюшка для создания новой ноды
